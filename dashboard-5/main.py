...
class Dashboard5(PPTXGenerator):

    sheet_name = 'Advisory Dashboard'
    ...
    separate_charts = {
        'separate-headline-metrics': '5-Headline-Metrics.pptx',
        'separate-revenue-and-earnings': '5-Revenue-Earnings-by-Major-Service-Area.pptx',
        'separate-key-metrics': '5-Key-Metrics.pptx',
        'separate-eba-earnings': '5-EBA-Earnings.pptx',
        'separate-rates-and-cs-headcount': '5-Rates-CS-Headcount-by-Geo.pptx',
        'separate-top-10-managed-clients': '5-Top-10-Managed-Clients.pptx',
    }
    ...
    def get_simple_fillers(self):
        ...
        return {
            ...
            lambda v: self.format_percent(min(v, .9999), prec=1, strip=False)+'%': [ # Max is 99.9%, if above display 99.9%, Always display percent with 1 decimal point 
                C('H3'), C('J3'),
                C('B5'), C('D5'), C('F5'),
            ],
            lambda v: '%sBPS' % self.format_float(abs(v), prec=0): [ # Max BPS change is 9999 in either direction 
                C('H4'), C('J4'),
                C('H5'), C('J5'),
            ]
        }
    ...
    def fill_blue_rect(self):
        ...
        sizes = zip(
            'BCDE',
            [self.get_cell(c) for c in C('B12', 'E12')],
            [self.get_cell(c) for c in C('B16', 'E16')],
        )
    ...
    def fill_rates_by_geography(self):
        ...
        # numerate x-Axis
        axis_e = self.E('rates-by-geography-chart-x-axis')
        x0 = self.get_element_coords(axis_e)[0]
        w = self.get_element_sizes(axis_e)[0] # x-Axis width
        labels = C('B38', 'AN38') # Periods
        step = w/len(labels) # step between periods
        # numerate x-Axis with periods
        for i, label in enumerate(labels):
            label_e = self.E(label)
            self.set_element_pos(label_e, x0+step*(i+.5)-self.get_element_sizes(label_e)[0]/2, None)
    ...
    def fill_top10(self):
        ...
        v = self.get_cell('B47') # max Axis scale
        values = [v*i/6 for i in range(6, 0, -1)]
        cells = ['B47'] + ['%s/6*B47' % i for i in range(5, 0, -1)] # Axis points between Max and Min
        ...
        max_val = self.get_cell('B47') # Top scale
    ...
    def fill_values(self):
        super(Dashboard5, self).fill_values()
